...
pts_listServers = function (servername, country, serverstatus, usersmin, usersmax, slotsmin, slotsmax, serverflagpw, serverflagcc, submit) {
  ...
  jQuery('select[name="serverflagcc"]').livequery(function () {
    ...
    if (servername) {
      jQuery('input[name="servername"]').val(servername);
    }
    if (country) {
      jQuery('select[name="country"]').val(country);
    }
    if (serverstatus) {
      jQuery('select[name="serverstatus"]').val(serverstatus);
    }
    if (usersmin) {
      jQuery('input[name="usersmin"]').val(usersmin);
    }
    if (usersmax) {
      jQuery('input[name="usersmax"]').val(usersmax);
    }
    if (slotsmin) {
      jQuery('input[name="slotsmin"]').val(slotsmin);
    }
    if (slotsmax) {
      jQuery('input[name="slotsmax"]').val(slotsmax);
    }
    if (serverflagpw) {
      jQuery('select[name="serverflagpw"]').val(serverflagpw);
    }
    if (serverflagcc) {
      jQuery('select[name="serverflagcc"]').val(serverflagcc);
    }
    //}
...
(function () {
  ...
  jQuery(document).ready(function () {
    ...
    jQuery('#public_servers_button').click(function () {
      pts_publicServers();
    });
    ...
    if (str.indexOf('Search Results') >= 0) {
      jQuery('a[href^="https://www.planetteamspeak.com/serverlist/result/server/ip/"]').each(function (index, element) {
        ...
        var ip = href.substr(href.lastIndexOf('/') + 1);
    ...
    jQuery('#ts3_link_params_button').click(function () {
      ...
      if ((params) && (params !== null) && (params !== '')) {
        localStorage.setItem('ts3_link_params', params);
        console.log(pts_name + ' Set advanced ts3server:// link parameters to: "' + localStorage.getItem('ts3_link_params') + '"');
        window.reload();
      } else {
        localStorage.removeItem('ts3_link_params');
        console.log(pts_name + ' Cleared advanced ts3server:// link parameters.');
        window.reload();
      }
    });
